...
| Layer | Result |
|---|---|
| Full `php artisan test` suite | **226 tests, 1,632 assertions** |
...
| Flutter `flutter test` (default run, Flutter 3.47.4) | **66 passed**; 3 live API suites skipped by design |
| Flutter live suites (`--tags live --run-skipped --concurrency=1`) | 3 of 3 against a seeded local API |
| GitHub Actions CI (every push) | Pint `--test`, full PHPUnit suite, `flutter analyze`, `flutter test`; green on `859503e` |
...
thumbnails, orphaned-file pruning, and content reports (submit,
resolve with takedown or removal, dismiss).
...
| A17 | Data flow diagrams added: level 1 (Figure 7, eight processes, sixteen stores D1 to D16) and level 2 for every process (Figures 8 to 15), generated by `docs/manuscript/figures/src/gen_dfd.py` from the routes, CMS actions, and scheduled commands. ERD renumbered to Figure 16, order states to Figure 17. | none (docs) | Chapter 3 Data flow section extended. |
| A18 | Content reports: buyers report published listings and visible reviews, farmer-sellers report reviews of their own shop (`POST /api/reports`, 10 per hour, one open report per target); Super Admins are notified in-app; the Super Admin resolves (optionally taking down the listing or removing the review through the extracted `TakeDownListingAction` / `RemoveReviewAction`) or dismisses, and the reporter gets a fixed notice. Own reports appear in the data export. No farmer-seller screen lists shop reviews yet, so the app offers review reporting on the buyer side only. | `859503e` | New process 9.0 and store D17 in the DFDs (Figures 7, 16); use case and context diagrams updated; ERD now Figure 17, order states Figure 18; test totals 226 / 1,632 and Flutter 66; Table 3 and Appendix F gain report cases; Chapter 1 module E mentions reports. |
