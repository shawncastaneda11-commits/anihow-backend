...
| A17 | Data flow diagrams added: level 1 (Figure 7, eight processes, sixteen stores D1 to D16) and level 2 for every process (Figures 8 to 15), generated by `docs/manuscript/figures/src/gen_dfd.py` from the routes, CMS actions, and scheduled commands. ERD renumbered to Figure 16, order states to Figure 17. The CMS Reports queue is left off: no route or app screen submits a report. | none (docs) | Chapter 3 Data flow section extended. |
